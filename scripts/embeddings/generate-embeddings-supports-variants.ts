...
async function generateEmbeddingsForSupportsVariants(options: {
  ...
  console.log('\n🚀 Génération des embeddings pour new_01_supports_variants\n');

  let query = supabase
    .from('new_01_supports_variants')
    .select('variant_slug, texte_vectorise, embedding');
  ...
  for (let i = 0; i < records.length; i++) {
    const { error: updateError } = await supabase
      ...
      .eq('variant_slug', records[i].variant_slug);
...
generateEmbeddingsForSupportsVariants({ forceRegenerate, limit }).catch(console.error);
