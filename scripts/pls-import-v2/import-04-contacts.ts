...
async function importContacts(dryRun: boolean = false) {
  console.log('\n🚀 Import Contacts Started\n');

  const csvPath = resolve(__dirname, '../pls-data/new_04_contacts-vue_leo.csv');
  const content = readFileSync(csvPath, 'utf-8');
  const data = parseCSV(content);

  console.log(`📊 Found ${data.length} contacts records\n`);
...
importContacts(dryRun).catch(console.error);
